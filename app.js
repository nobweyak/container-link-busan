...
async function matchList() {
  ...
  let rows = [];
  try { await connect(); const timeout=new Promise((_,reject)=>setTimeout(()=>reject(new Error('Firestore timeout')),7000)); const result = await Promise.race([getDocs(query(collection(db, 'containerRequests'), where('status', '==', 'open'))),timeout]); rows = result.docs.map((row) => ({ id: row.id, ...row.data() })).filter((item) => !state.hidden.has(item.id) && state.types.includes(item.type) && state.sizes.includes(item.size)); } catch(error) { console.error(error); say('요청을 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.'); }
